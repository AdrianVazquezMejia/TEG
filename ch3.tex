En esta capitulo se describirá el diseño de la red, detalles de software y hardware de los nodos.
...
Dentro de los funcionamiento descritos anteriormente, se tiene que el funcionamiento descrito en la sección \ref{primera} se concentra en el algoritmo de optimización de la topología de la red, sin tomar en consideración el hardware de implementación. Por otro lado en la sección  \ref{segunda}, se provee una aproximación más realista, donde existen  especificaciones del hardware, sin embargo, no hay justificación para el enrutamiento usado y las medidas de rendimiento aplicadas no son de interés para el área industrial. Por último, se tiene que la ESP-MESH parecía  brindar extensibilidad y rápido desarrollo, no obstante, posee la desventaja de la necesidad de un enrutador WiFi  para la formación, que brinde las direcciones IP a la red mallada, lo que deriva en un centralización, pues si el router falla, la red perece. 
...
\espn usa una trama de acción (Trama de control) específica de proveedor para transmitir datos. La tasa de transmisión por defecto es de 1 Mbps y formato de la trama es como se muestra en la tabla \ref{Tabla: espn}.
\begin{table}[H]
\resizebox{15cm}{!}{
...
Las rutas no son determinadas por la red, si no por el usuario que la implemente. Además, son estáticas y no se reasignan automáticamente, estas rutas permanecen en una tabla unidimensional, con los identificadores de los nodos destinos. El enrutamiento está estrechamente relacionado con los esclavo Modbus, y están representados por las posiciones en la tabla de enrutamiento.
...
\begin{figure}[H]
\centering
\includegraphics[scale=1]{dcdc.jpg}\label{dcdcF}
\caption{Tarjeta con regulador DC-DC para alimentación}
\end{figure}
...
Este chip soporta hasta 32 dispositivos, pero teniendo en cuenta que el uso de nodos distribuye la carga de los esclavos, la utilización de repetidores se vería disminuida, y se soportarían 32 dispositivos por cada nodo.
...
\caption{Ilustración del paquete para el desarrollo de aplicaciones en el ESP32 }
\end{figure}
...
\item Si el PCB de la antes tiene que ser colocado en una tarjeta huesped, es recomendada la opción 4, que no posee plano de cobre bajo la antena, así hayan algunas pérdidas del rendimiento.\label{recomendacion}
...
Por lo tanto se tomo la recomendación \ref{recomendacion} eligiendo la opción 4 para el diseño.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{maxim.png}\label{Fig  2: MAX y UART}
...
\begin{figure}[H]
\centering
\includegraphics[scale=1]{esquematico.eps}\label{esquematico}
\caption{Esquemático del nodo}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=1]{pcbF.eps}\label{pcbF}
\caption{Diagrama de circuito impreso capa superior}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=1]{pcbB.eps}\label{pcbB}
...
\subsubsection{Ilustración de los nodos}
 ...
 A continuación se presentan imágenes de los nodos donde se implementó la red.
 
 \begin{figure}[H]
 \centering
 \includegraphics[scale=0.2]{images/foto1.jpeg}\label{master Node}
 \caption{Hardware del nodo maestro.}
 \end{figure}
 
  \begin{figure}[H]
 \centering
 \includegraphics[scale=0.2]{images/foto2.jpeg}
...
Se toma la topología \ref{topo} como base para todas las pruebas, excepto, por supuesto, la prueba de variación de topología.  Se colocó el nodo 150 como el nodo que posee el maestro conectado,  el nodo 151 para ejecutar  reenvío de datos y los nodo 153 y 152 con  un esclavo Modbus conectados.
...
\hline 
\end{tabular} 
\caption{Condiciones para las pruebas de rendimiento, la prueba cuatro se asocia a la variación de las topologías.}
\end{table}
